...
// is reading order (shared/sheet/reading-order.ts), the order a report
// reads the sheet in too.
import { dataOf, inReadingOrder } from '@digsite/shared';
...
function imagesInReadingOrder(elements: readonly ElementLike[]): ElementLike[] {
  return inReadingOrder(
    elements.filter((el) => !el.isDeleted && dataOf(el)?.kind === 'image'),
  );
}
