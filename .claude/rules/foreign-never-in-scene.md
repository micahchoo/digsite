...
# sheet: a foreign claim is never an owned scene element

The scene of a sheet holds only what that sheet owns: its
...
under test — zero leaks in 62 scenes — and still produced a pointer
fallthrough bug in the former canvas implementation. The separate SVG
overlay removed that failure mode.
...
Verify with the foreign overlay e2e scenarios: no element with `foreign` in any
