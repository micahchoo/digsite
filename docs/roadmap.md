...
| C11 | **The paint path still grows, slowly.** 0.09 MB a painted image, linear from 300 to 1,500 images in `repro-paint-ladder-import.ts`; about 1.8 GB over a 20,000-file import, contained today only by the worker retiring. The harness's pass mark (under 128 MB at 600) hides it. | bisect the harness step by step, judged by the slope from 300 to 1,500 | whatever the bisect names; then make the harness fail on slope |
