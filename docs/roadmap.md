...
| 4 | The machine suggests, never decides | suggested connections, labels and near-duplicates from embeddings, each accepted by a person | in progress: Find by meaning, More like this, and "Looks like" on a sheet picture (Bring here, then connect). Near-duplicates: server route done (`GET /boards/:id/duplicates`), UI open. Open: label suggestions |
...
| 8 | Camera formats | HEIC and RAW through the folder import: a phone folder imports with no skip, or each skip explained | in progress. Phase 1: HEIC and RAW files are listed and each is skipped with its reason; before this they vanished from the count. Phase 2, decoding: the bundled libheif has no HEVC decoder (an iPhone HEIC fails), and libvips reads a NEF's first TIFF directory, which is its 160×120 thumbnail, so RAW needs its embedded full-size JPEG, never a plain sharp decode |
